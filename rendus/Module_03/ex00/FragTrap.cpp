...
FragTrap::~FragTrap() {
    std::cout << "FR4G-TP " << name << " is DEAD. Death Eaters flee." << std::endl;
}
...
void FragTrap::rangedAttack(std::string const & target) {
    std::cout << "FR4G-TP " << name << " range-attacked " << target << " like a coward!" << std::endl;
}
...
void FragTrap::takeDamage(unsigned int amount) {
	...
	if (this->hit_points < 0)
		this->hit_points = 0;
    std::cout << "FR4G-TP " << name << " tragically lost " << amount << " of HP!" << std::endl;
}
